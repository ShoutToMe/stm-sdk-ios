...
typedef enum eDL_URLRequestMethod
{
    DL_URLRequestMethod_Get = 0,
    DL_URLRequestMethod_Post,
    DL_URLRequestMethod_Put,
    DL_URLRequestMethod_Delete
} tDL_URLRequestMethod;
...
#define DL_URLSERVER_DEFAULT_CONTENT_TYPE   @"application/x-www-form-urlencoded"
...
//issue an HTTP request with caching
//	same as above but with 1 extra parameter:
//	strContentType:
//		content type for a post (e.g., @"application/json; charset=utf-8")
//      if not supplied, default will be: @"application/x-www-form-urlencoded"
...
//issue an HTTP request with content type
//	same as above but with 1 extra parameter:
//	dictHeaderRequests:
//      additional HTTP header items user can add to requests (API key for instance)
//		if nil, none are added
