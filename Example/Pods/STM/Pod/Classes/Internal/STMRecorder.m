...
@interface STMRecorder ()
@property (nonatomic, assign) RecorderState *state;
@property (atomic) STMVad *vad;
@property float bufferLength;
@end


@implementation STMRecorder {
...
static void audioQueueInputCallback(void* data,
    ...
    if (size > 0) {
        ...
        @autoreleasepool {
            STMRecorder* recorder = (__bridge STMRecorder*)data;
...
static void MyPropertyListener(void *userData, AudioQueueRef queue, AudioQueuePropertyID propertyID) {
    if (propertyID == kAudioQueueProperty_IsRunning)
        NSLog(@"Queue running state changed");
}
...
- (BOOL)stop {
    ...
    if (err) {
        NSLog(@"[STM] ERROR: could not flush audio queue (%d)", err);
    }
    err = AudioQueuePause(self.state->queue);
    if (err) {
        NSLog(@"[STM] ERROR: could not pause audio queue (%d)", err);
    }
...
//    debug(@"Recorder: updating power");
    ...
    if (err) {
        NSLog(@"Error while reading meters %d", err);
        return;
    }
...
-(void)dispatchNewPower:(float)power {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:STM_NOTIFICATION_AUDIO_POWER_CHANGED object:newPower];
}
...
- (void)initialize {
    ...
    NSLog(@"AudioQueue buffer size: %d bytes", bytes);

    for (int i = 0; i < kNumberRecordBuffers; i++) {
        err = AudioQueueAllocateBuffer(state->queue, bytes, &state->buffers[i]);
        if (err) {
            NSLog(@"error while allocating buffer %d", err);
        }
    }
    ...
    if (err) {
        NSLog(@"error while adding listener buffer %d", err);
    }
