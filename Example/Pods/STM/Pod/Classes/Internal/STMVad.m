...
@implementation STMVad {
    stm_s_wv_detector_cvad_state *vad_state;
    FFTSetup fft_setup;
}
...
-(void) gotAudioSamples:(NSData *)samples {
    ...
    for(int sample_offset=0; sample_offset+self->vad_state->samples_per_frame < size/2; sample_offset+=self->vad_state->samples_per_frame){
        ...
        int detected_speech = stm_wvs_cvad_detect_talking(self->vad_state, bytes+sample_offset, fft_mags);
...
-(id) init {
    NSLog(@"STMVad init");
    self = [super init];
    int vadSensitivity = fmin(100,fmax(0,[STM recordingSystem].vadSensitivity)); //must be between 0 and 100
    int vadTimeout = [STM recordingSystem].vadTimeout;
    
    self->vad_state = stm_wv_detector_cvad_init(16000,vadSensitivity,vadTimeout);
...
-(void) dealloc {
    NSLog(@"Clean STMVad");
    stm_wv_detector_cvad_clean(self->vad_state);
}
